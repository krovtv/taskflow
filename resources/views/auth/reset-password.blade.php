...
<div x-data="{ loading: false, showPassword: false }">
    <h2 class="text-xl font-extrabold text-kvnavy dark:text-white mb-1 tracking-tight">Redefinir senha</h2>
    <p class="text-sm text-slate-400 mb-2 font-medium">Digite o código recebido no e-mail e sua nova senha.</p>
    ...
    <form method="POST" action="{{ route('password.update') }}" class="space-y-5" @submit="loading = true">
        ...
        <div>
            <label class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Código de recuperação</label>
            <div class="relative">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input :type="showPassword ? 'text' : 'password'" name="password" required autocomplete="new-password" minlength="6"
                       placeholder="• • • • • • • •"
        ...
        <div>
            <label class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Confirmar nova senha</label>
            <input type="password" name="password_confirmation" required autocomplete="new-password" minlength="6"
                   placeholder="• • • • • • • •"
                   class="w-full border border-slate-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm bg-slate-50 dark:bg-gray-800 focus:bg-white dark:focus:bg-gray-800 focus:border-kvteal focus:ring-2 focus:ring-kvteal/20 transition-all outline-none placeholder:text-slate-300 dark:placeholder:text-slate-500">
        </div>
        ...
        <button type="submit" :disabled="loading"
                ...
                x-text="loading ? 'Redefinindo...' : 'Redefinir senha'">
        </button>
    </form>

    <p class="text-center text-sm text-slate-500 mt-6 font-medium">
        <a href="{{ route('login') }}" class="text-kvteal font-bold hover:underline transition-colors">Voltar ao login</a>
    </p>
</div>
@endsection
